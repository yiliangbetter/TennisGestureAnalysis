        ...
        for i, frame in enumerate(frame_sequence):
            ...
            if results.pose_landmarks:
                ...
                key_joints = [
                    ...
                    ('right_shoulder', results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value]),
                    ('left_elbow', results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_ELBOW.value]),
                    ('right_elbow', results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_ELBOW.value]),
                    ...
                    ('left_ankle', results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_ANKLE.value]),
                    ('right_ankle', results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_ANKLE.value]),
                ]

                key_points = [(joint[0], joint[1].x, joint[1].y) for joint in key_joints]

                # Calculate angles between joints
                angles = self._calculate_joint_angles(results.pose_landmarks)
                ...
                if i > 0:
                    ...
                    if prev_results:
                        prev_landmarks = np.array([
                            [lm.x, lm.y, lm.z] for lm in prev_results.pose_landmarks.landmark
                        ])
                        ...
                        if i >= 2:
                            ...
                            if prev_prev_results:
                                prev_velocity = prev_landmarks - np.array([
                                    [lm.x, lm.y, lm.z] for lm in prev_prev_results.pose_landmarks.landmark
                                ])
    ...
    def _calculate_joint_angles(self, landmarks) -> List[float]:
        ...
        for triplet in angle_triplets:
            ...
            try:
                ...
                point1 = np.array([landmarks.landmark[p1].x, landmarks.landmark[p1].y])
                point2 = np.array([landmarks.landmark[p2].x, landmarks.landmark[p2].y])
                point3 = np.array([landmarks.landmark[p3].x, landmarks.landmark[p3].y])
    ...
    def _extract_trajectory(self, frame_sequence: List[np.ndarray], current_idx: int) -> List[Tuple[float, float]]:
        ...
        for i in range(max(0, current_idx - trajectory_length), current_idx + 1):
            ...
            if results.pose_landmarks:
                ...
                wrist_x = results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].x
                wrist_y = results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].y
